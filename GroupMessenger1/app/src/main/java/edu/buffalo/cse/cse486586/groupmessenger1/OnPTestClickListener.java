...
public class OnPTestClickListener implements OnClickListener {
    ...
    private static final String TAG = "query";//OnPTestClickListener.class.getName();
    ...
    public OnPTestClickListener(TextView _tv, ContentResolver _cr) {
        Log.e("Debugg","ptestlistener constructor");
        mTextView = _tv;
    ...
    private ContentValues[] initTestValues() {
        ...
        Log.e("Debugg","inittest called");
        return cv;
    }

    @Override
    public void onClick(View v) {
        Log.e("Debugg","onclick called");
        if (testInsert()) {
    ...
    private boolean testQuery() {
        try {
            for (int i = 0; i < TEST_CNT; i++) {
                ...
                /*
                Log.e(TAG, String.valueOf(returnKey.equals(key)) );
                Log.e(TAG, String.valueOf(returnKey.length()) );
                Log.e(TAG, String.valueOf(key.length()) );
                Log.e(TAG, String.valueOf(returnValue.trim().length()) );
                Log.e(TAG, String.valueOf(val.length()) );
                */
                if (!(returnKey.equals(key) && returnValue.trim().equals(val))) {

                    /*
                    Log.e(TAG, returnKey + " " + returnValue + "pair");
                    Log.e(TAG, key +" " + val);
                    */
